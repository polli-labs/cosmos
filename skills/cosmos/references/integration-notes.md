...
- Cosmos core depends on dependency-light `polli-typus` 0.8 without visualization,
  tracking, or ML extras. SDK consumers may use `PreviewRect.typus_bbox` as the
  canonical `BBoxXYWHNorm` view when it is representable.
...
- `cosmos.sdk.video` is the shared typed video metadata/timeline/frame extraction surface for
  downstream repos that need exact decoded-frame identities or RGB frames without carrying
  local Decord or ad hoc FFmpeg code.
...
- Keep the eight `PreviewRect` schema-v1 fields and their serialized byte shape
  unchanged. Pixel fields remain authoritative after clamp/forced-even execution
  math; normalized fields remain their lossy projection. A `typus_bbox` value is
  derived only when Typus accepts those exact normalized values, and `None`
  explicitly covers zero extent or lossy out-of-bounds rounding without repair.
...
- Preserve typed resolver failures, OS-level launch failures (including missing or
  permission-denied executables), timeouts, and non-zero process exits across video probe,
  timeline, and FFmpeg decode paths. `COSMOS_VIDEO_FFMPEG_TIMEOUT` is an optional positive
  finite seconds bound; invalid or absent values retain unbounded compatibility behavior.
- Bind cross-repo frame identity with `VideoFrameTimeline` integer PTS ticks plus its stream
  time-base numerator/denominator. Those ticks are literal frame `pts`; estimated
  timestamps, nominal FPS, and rounded seconds are display/selection aids, not durable
  identity joins.
...
- Use `cosmos process` as the canonical ingest -> crop orchestration command;
  `cosmos pipeline` is retired.
...
- video substrate contracts (`VideoProbe`, `VideoFrameTimeline`, `RgbFrame`, exact timeline
  semantics, extraction semantics, resolver errors)
