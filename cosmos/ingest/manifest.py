...
class ManifestParser:
    ...
    def _parse_manifest(self) -> None:
        ...
        for elem in root:
            try:
                ...
                start_time = datetime.strptime(elem.attrib["InStr"], "%H:%M:%S.%f %m/%d/%Y")
    ...
    def get_clips(self) -> list[ClipInfo]:
        ...
        return sorted(self._clips.values(), key=lambda x: x.start_epoch)
...
def find_manifest(base_dir: Path) -> Path | None:
    ...
    logger.debug(
        f"Searching for manifest in received base_dir: '{base_dir}' (type: {type(base_dir)})"
    )
